package com.darkkeeper.themaze.Utils;

/**
 * Created by andreipiatosin on 7/27/15.
 */
public class Constants {
    public static final float APP_WIDTH = 1920;
    public static final float APP_HEIGHT = 1080;

    public static final int MAZE_DEFAULT_WIDTH = 8;
    public static final int MAZE_DEFAULT_HEIGHT = 4;

    public static final float BOTTOM_BAR_HEIGHT = 100;
    public static final float BOTTOM_BAR_ICON_SIZE = 90;
    public static final float BOTTOM_BAR_ICON_PADDING_LEFT = 20;
    public static final float BOTTOM_BAR_ICO_PADDING_BOTTOM = 10;

    public static boolean[][] cells;
}
